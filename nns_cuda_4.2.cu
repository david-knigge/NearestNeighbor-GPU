...
void NSS(const list_t& L, size_t t, callback_list_t f)  {
    ...
    *l_size = L.size();
    ...
    for (i = 0; i <iterations; i = i+NUMBER_OF_THREADS){
      ...
      *vec = i;
      nns_kernel<<< (NUMBER_OF_THREADS + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(vec, vecsd, ret_vecd, vecd_size, l_sized, thres);
    }
